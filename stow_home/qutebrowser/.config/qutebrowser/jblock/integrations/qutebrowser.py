...
# These calls need to be duplicated to fool mypy into thinking they are real
...
JBLOCK_FREQ_TMP = config.datadir / ".jblock-freq.tmp"
...
def jblock_save_frequency():
	...
	with open(JBLOCK_FREQ_TMP, 'wb') as f:
		pickle.dump(freq, f)
		f.flush()
		os.fsync(f.fileno())
	os.rename(JBLOCK_FREQ_TMP, JBLOCK_FREQ)
...
def jblock_intercept(info: interceptor.Request):
	...
	if jblock_buckets is None or psl is None:
		message.error("Critical error loading buckets. Bailing.")
		return
	...
	blocking_num += 1
	heapq.heappush(slowest_urls, (time_change, url)) # type: ignore
